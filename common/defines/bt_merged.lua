...
NDefines.NAI.DIPLOMATIC_ACTION_IMPROVE_RELATIONS_BEFRIEND_FACTOR = 150 -- BT Changed to make ai better from 50
NDefines.NAI.DIPLOMATIC_ACTION_IMPROVE_RELATIONS_ALLY_FACTOR = 150 -- BT Changed to make ai better from 50
...
NDefines.NAI.DIPLOMATIC_INTEREST_DISTANCE = 160 -- BT Changed slight increase to get more people to ally with
NDefines.NAI.EXTRA_SURPLUS_WHEN_NEEDING_BUILDINGS = 0.25 -- BT changed to hopefully get ai to build more Harbivore
NDefines.NAI.FIRE_ADVISOR_LOAN_REQUIREMENT = 2 -- BT changed because loans are doubled in BT from 5
NDefines.NAI.FORT_CAPITAL_DESIRE = 0.5  -- Harbivore changed back to vanilla's 0.5 moved to buildings so we can have if statements 
...
NDefines.NAI.MIN_SCORE_TO_CONCENTRATE_DEVELOPMENT = 2 --BT Ai seem to alway concentrate dev changed from 1.5 Harbivore
NDefines.NAI.MINIMUM_STRENGTH_TO_AVOID_UNCONDITIONAL_SURRENDER = 30 -- BT increased to hopefully have ai chose to lose this to win another day Harbivore
NDefines.NAI.MISSIONARY_MAINTENANCE_SHARE = 0.1 -- BT changed from 0.2 to make ai make better choices
NDefines.NAI.UNCONDITIONAL_SURRENDER_HOPELESS_STRENGTH_COMPARISON = 3 -- BT decreased to hopefully have ai chose to lose this to win another day Harbivore
NDefines.NAI.UNCONDITIONAL_SURRENDER_WARSCORE_LIMIT = -50 -- BT decreased to hopefully have ai chose to lose this to win another day Harbivore
NDefines.NAIEconomy.BASE_SAVINGS_PRIORITY = 1 -- BT changed from 0.5 to make ai make better choices
NDefines.NAIEconomy.BASE_SAVINGS_PRIORITY_WARTIME = 0.1 -- BT changed from 0 to make ai make better choices
NDefines.NAIEconomy.LOAN_REPAYMENT_SAVINGS_PRIORITY = 2 -- BT changed because loans are doubled in BT from 1 
...
NDefines.NCountry.CORE_OVERSEAS = 0.25					-- BT changed from 0p5 as part of coring overhaul
...
NDefines.NCountry.INNOVATIVENESS_BEHIND_IN_TECH_PENALTY = -0.05			-- BTAddOn changed from 0p03 Trying to make being ahead of time somewhat more useful
...
NDefines.NCountry.INNOVATIVENESS_FIRST_PICKED_IDEA = 1.0			-- BTAddOn changed from 1p0 Trying to make being ahead of time somewhat more useful
NDefines.NCountry.INNOVATIVENESS_FIRST_REACHED_TECH_LEVEL = 3.0			-- BTAddOn changed from 2p0 Trying to make being ahead of time somewhat more useful. Vanilla buffed it following their lead.
NDefines.NCountry.INNOVATIVENESS_MAX = 200.0
...
NDefines.NCountry.MONTHS_TO_CORE = 120					-- BT changed from 36 as part of Coring overhaul
NDefines.NCountry.NEIGHBOURBONUS = -0.05			-- With the new institution tech system these neighbour bonuses have become way too powerful after a few decades of neighbouring a coloniser anyone can match max tech in the world reducing from 5 percent to 2 and a half percent for a start
...
NDefines.NCountry.PS_ADD_ACCEPTED_CULTURE = 200 --BT changed from 100 as part of the Accepted Culture Slots overhaul. There's now an infinite amount of accepted culture slots, and instead, you pay a lot more for each accepted culture you make. Putting it at 200, with a base cost increase of +50%, means it costs 300 dip points at the start of the game to accept a single culture. With Humanist ideas, this goes down to 200, and as the game goes on, it loses an additional 100 in cost. And then, there's additional modifiers to be gained in policies, national ideas, as well as government reforms to push this down to the minimum. //Dub
...
NDefines.NCountry.PS_IMPROVE_PROVINCE_BASE = 25 			-- from 50 temp for dev points
...
NDefines.NCountry.RETURN_CORE_FAVOR_COST_PER_DEV = 1.5			-- from 3
...
NDefines.NDiplomacy.VOLUNTARY_JOIN_HRE_AUTHORITY_PER_DEVELOPMENT = 0.05		-- from 0p1
NDefines.NDiplomacy.LEAVE_HRE_AUTHORITY_PER_DEVELOPMENT = -0.2				-- from 0p2
--Below appears to be cut. //Dub
...
--NDefines.NDiplomacy.HRE_AUTHORITY_PROVINCE_JOIN = 0.05			-- from 0p1. More provinces equals more IA, hence counterbalance by less IA gain. Removed from the game
...
NDefines.NDiplomacy.HRE_HERETIC_PENALTY = -0.004				-- Beyond Typus, reduced from -0.01
NDefines.NDiplomacy.HRE_PRINCE_AUTHORITY_THRESHOLD = 40			-- increased from 25
NDefines.NDiplomacy.INTEGRATE_VASSAL_MIN_YEARS = 25 -- (From 10) BT added to give vassals some staying power and because we increased the coring time to average 10 years.
NDefines.NDiplomacy.IMPERIAL_AUTHORITY_FROM_PRINCES = 0.06			-- to 0.045 reduced from 0.1 #Increased to 0.6 to match vanilla numbers
...
NDefines.NDiplomacy.REVANCHISM_MONTHLY_DECAY = 0.4               -- BT Halved to help nations that have had land taken
...
NDefines.NGovernment.CONCENTRATE_DEVELOPMENT_CAPITAL_PROPORTION = 0.5 --BT changed from 0.8 so more dev ends up in places other than the capital too many nations had 100+ dev capitals
NDefines.NGovernment.DEVASTATION_PER_DEV_STOLEN = 2.5 --from 5
NDefines.NGovernment.GOVERNMENT_REFORM_COST_CENTRALIZE_STATE = 0 --BT changed from 50. This button was never used by anyone, and we need an admin point sink. //Dub
...
NDefines.NGraphics.EQUATOR_COORDINATE_Y = 1009 -- Dub changed from 671. This must be counting from the bottom else this makes no sense. I had a hard time finding the equator, and I get the feeling I'm slightly off.
NDefines.NGraphics.MIN_LATITUDE_FOR_MAP = -58.5 -- Dub changed from 60p0 in accordance with Draiocht's announcement comment
...
NDefines.NMilitary.CAVALRY_BREAKTHROUGH = 0.2	-- Dub changed from 0p0. Cav in the later game is incredibly useless - no fire damage, dies in fire phase, etc. etc.. This would help them fill a niche - when defeating an infantry unit, you have a 20% chance of pulling the arty unit to the front row.
